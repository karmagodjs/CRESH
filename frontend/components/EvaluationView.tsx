    ...
    <div className="h-full overflow-y-auto p-6 sm:p-8 space-y-8 bg-cri-bg text-cri-textPrimary max-w-6xl mx-auto">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] font-mono text-cri-info font-medium px-2 py-0.5 rounded bg-cri-surfaceSecondary border border-cri-border">
            44 / 44 Invariant Assertions Passed
          </span>
        </div>

        <div className="rounded-lg border border-cri-border bg-cri-surface overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-cri-border bg-cri-surfaceSecondary text-cri-textMuted font-mono">
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Quality Metric</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Baseline (Phase 5)</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-cri-textPrimary">Hardened (Phase 6)</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Empirical Delta</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-cri-border">
                {PHASE_6_BENCHMARKS.map((row, idx) => (
                  <tr key={idx} className="hover:bg-cri-surfaceSecondary/50 transition-colors">
                    <td className="p-3 font-medium text-cri-textPrimary">{row.metric}</td>
                    <td className="p-3 text-cri-textMuted font-mono">{row.baseline}</td>
                    <td className="p-3 font-semibold text-cri-success font-mono">{row.hardened}</td>
        ...
        <div className="rounded-lg border border-cri-border bg-cri-surface overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-cri-border bg-cri-surfaceSecondary text-cri-textMuted font-mono">
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Configuration</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">Recall@5</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">Recall@10</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">MRR@10</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">Precision@5</th>
                  <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">nDCG@10</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-cri-border font-mono text-[11px]">
                {RETRIEVAL_ABLATION.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-3 font-sans font-medium text-cri-textPrimary">{row.config}</td>
                      <td className="p-3 text-right">{row.recall5}</td>
                      <td className="p-3 text-right text-cri-success">{row.recall10}</td>
