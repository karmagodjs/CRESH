  ...
  const pipelineSteps = [
    {
      ...
      type: "Guard",
      color: "border-cri-orange text-cri-orange",
    },
    {
      step: "02",
      ...
      type: "Analysis",
      color: "border-cri-blue text-cri-blue",
    },
    {
      step: "03",
      ...
      type: "Retrieval",
      color: "border-cri-borderLight text-cri-paper",
    },
    {
      step: "04",
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {pipelineSteps.map((st, idx) => (
            <div
              key={idx}
              className="p-4 rounded border border-cri-border bg-cri-surface hover:bg-cri-surfaceActive transition-colors relative"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-xs text-cri-textMuted font-semibold">
                  STAGE {st.step}
                </span>
                <span className={`text-[10px] px-2 py-0.5 rounded border uppercase font-medium ${st.color}`}>
                  {st.type}
                </span>
              </div>
              <h3 className="text-sm font-semibold text-cri-paper mb-1.5">{st.title}</h3>
              <p className="text-xs text-cri-textMuted leading-relaxed">{st.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 rounded border border-cri-border bg-cri-surface space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-cri-paper">
              Structure-Aware Chunking
            </h3>
          </div>
          <ul className="text-xs text-cri-textMuted space-y-1.5 list-disc pl-4 leading-relaxed">
            <li>Target size: 400 tokens (~1,600 chars) with 80-token overlap.</li>
            <li>Section boundary preservation prevents splitting mid-equation.</li>
        ...
        <div className="p-4 rounded border border-cri-border bg-cri-surface space-y-2">
          ...
          <ul className="text-xs text-cri-textMuted space-y-1.5 list-disc pl-4 leading-relaxed">
            <li>Qdrant Vector Database isolates vectors strictly by <code className="text-cri-paper text-[11px]">document_id</code> payload filter.</li>
